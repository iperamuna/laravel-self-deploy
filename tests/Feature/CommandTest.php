...
use Iperamuna\SelfDeploy\Console\Commands\CreateDeploymentFile;
...
it('preserves existing config structure when updating', function () {
    ...
    $command = new CreateDeploymentFile;
...
it('ensures the end.blade.php summary header is a single line', function () {
    $path = realpath(__DIR__.'/../../resources/views/end.blade.php');

    if (! $path || ! File::exists($path)) {
        // Try fallback if test is run from project root instead of package root
        $path = base_path('packages/iperamuna/laravel-self-deploy/resources/views/end.blade.php');
    }

    if (! $path || ! File::exists($path)) {
        // Just verify we can find it somehow or skip
        $this->markTestSkipped('Could not find end.blade.php for content check');
    }

    $content = File::get($path);

    // Check that the echo command for the summary header doesn't contain a literal newline between the colon and the header variable
    expect($content)->toContain('.${LEVEL}: ${SUMMARY_HEADER}"');
    expect($content)->not->toContain('.${LEVEL}:'.PHP_EOL.'${SUMMARY_HEADER}"');
});
